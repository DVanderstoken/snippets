...
@Injectable()
export class GeolocationService extends GoogleMapsAPIWrapper {
  private __geocoder: any;
  ...
  public getCity(position: Geolocation): Observable<string> {
    const latLng = { lat: position.latitude, lng: position.longitude };

    return Observable.create(observer => {
      this.__geocoder.geocode({ location: latLng }, (result, status) => {
        if (status === google.maps.GeocoderStatus.OK) {
          if (result[0] != null) {
            console.log(
              'GeolocationService.__getCity(position: GeoLocation = { location : { lat: ' +
                position.latitude +
                ', lng: ' +
                position.longitude +
                '}}) return : ' +
            ...
            observer.next(result[0].address_components[1].long_name);

            observer.complete();
          } else {
            console.log(
              'GeolocationService.__getCity(position: GeoLocation) return : No data found.'
            );
            observer.next({});
            observer.complete();
          }
