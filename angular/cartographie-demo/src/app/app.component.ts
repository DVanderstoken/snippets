...
import { TrackingRecords } from './mock-datas/tracking-records.model';
...
export class AppComponent implements OnInit {
  ...
  private __ships: string[] = ["Amborella", "Jules Garnier II"];
  private __shipDatas: TrackingRecords[];
  ...
  selectedShip(shipName) {
    this.shipTrackingService
      .getTrackingNodesForShip(shipName)
      .subscribe(trackRecord => {
        console.log(trackRecord);
        this.__osmmap.removeLayer(this.__cityMarker);
        this.__cityMarker = L.marker([
          trackRecord.lat,
          trackRecord.lon
        ]).addTo(this.__osmmap);
        this.__osmmap.setView(
          new L.LatLng(trackRecord.lat, trackRecord.lon),
          12,
          { animation: true }
        );
      })
